#!/usr/bin/env python3
"""LU 工况下 freeboard secondary-air 审计。

目标：
1. 用整炉归一化坐标 `xi≈0.6` 对齐文献中的二次风位置
2. 扫描二次风量，检查 O2/T spike 与 CO/H2 dip 是否出现
3. 保持 shared freeboard dev config 不变，仅输出开发证据
"""
...
TARGET_XI_EXIT = 1.00
SECONDARY_O2_FRACS = (0.00, 0.05, 0.10, 0.15, 0.20)
AIR_N2_O2 = 3.76
...
def _combined_wet_profiles(reactor: Reactor, result: dict) -> tuple[list[float], dict[str, list[float]], list[float]]:
    ...
    assert len(x) == len(t) == len(wet["CO"]) == n_bed + len(result["freeboard_T_profile"])
    return x, wet, t
...
def main() -> int:
    ...
    ref = {
        ...
        "exit_CO": _ref_at(node, "CO_mol_wet", TARGET_XI_EXIT),
        "exit_CO2": _ref_at(node, "CO2_mol_wet", TARGET_XI_EXIT),
        "exit_H2": _ref_at(node, "H2_mol_wet", TARGET_XI_EXIT),
        "exit_CH4": _ref_at(node, "CH4_mol_wet", TARGET_XI_EXIT),
    }
    ref["dCO"] = ref["CO_060"] - ref["CO_045"]
    ref["dH2"] = ref["H2_060"] - ref["H2_045"]

    print("=" * 176)
    print("LU freeboard secondary-air audit")
    print("=" * 176)
    print("temperature note: T@xi=0.60 is shown for context only; current score excludes it until Fig. 7.4 temperature curve is re-digitized.")
    print(
        "ref @xi=0.60: "
    ...
    print("-" * 176)
    print(
        f"{'secO2/prim':>10} {'O2_sec':>8} {'seg':>4} {'xi_seg':>8} {'O2@0.60':>8} {'T@0.60':>8} "
        f"{'CO@0.60':>8} {'CO2@0.60':>9} {'H2@0.60':>8} {'dCO':>8} {'dH2':>8} "
        f"{'Texit':>8} {'COx':>8} {'CO2x':>8} {'H2x':>8} {'CH4x':>8} {'score':>8}"
    )
    print("-" * 176)

    for frac in SECONDARY_O2_FRACS:
        cfg = build_phase2_htw_lu_freeboard_reactor_config()
        sec_o2 = float(cfg.O2_feed) * float(frac)
        cfg.freeboard_secondary_injection_xi = TARGET_XI_SPIKE
        cfg.freeboard_secondary_O2_mol_s = sec_o2
        cfg.freeboard_secondary_N2_mol_s = sec_o2 * AIR_N2_O2
        cfg.freeboard_secondary_H2O_mol_s = 0.0
        cfg.freeboard_secondary_T_K = cfg.T_inlet
        ...
        values = {
            ...
            "exit_CO": _interp_profile(x, wet["CO"], TARGET_XI_EXIT),
            "exit_CO2": _interp_profile(x, wet["CO2"], TARGET_XI_EXIT),
            "exit_H2": _interp_profile(x, wet["H2"], TARGET_XI_EXIT),
            "exit_CH4": _interp_profile(x, wet["CH4"], TARGET_XI_EXIT),
        }
        ...
        print(
            f"{frac:>10.2f} {sec_o2:>8.3f} {str(seg):>4s} {xi_seg:>8.3f} {values['O2_060']:>8.3f} {values['T_060']:>8.1f} "
            f"{values['CO_060']:>8.3f} {values['CO2_060']:>9.3f} {values['H2_060']:>8.3f} {values['dCO']:>8.3f} {values['dH2']:>8.3f} "
            f"{values['exit_T']:>8.1f} {values['exit_CO']:>8.3f} {values['exit_CO2']:>8.3f} {values['exit_H2']:>8.3f} "
            f"{values['exit_CH4']:>8.3f} {score:>8.3f}"
        )
        print(
            f"{'':>10} {'':>8} {'':>4} {'':>8} {'':>8} {'':>8} {'':>8} {'':>9} {'':>8} {'':>8} {'':>8} "
    ...
    print("  - 目标不是直接最优出口，而是先复现 xi≈0.6 的 O2 回升与 CO/H2 dip 指纹。")
    print("  - 当前更高置信的局部判据是 O2、CO、CO2、H2；T@xi≈0.6 仅作参考。")
    print("  - 若只抬高 Texit 而没有局部 O2 spike / CO-H2 dip，则说明二次风量或混合位置仍不对。")
    return 0
